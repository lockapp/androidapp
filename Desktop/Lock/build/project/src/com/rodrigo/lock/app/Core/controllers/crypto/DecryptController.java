...
import com.rodrigo.lock.app.Utils.*;
...
public class DecryptController extends CryptoController {
   ...
   // String destFileDir;
    ...
    //para see image
    public DecryptController(File inFile, String pass, Accion accio, long offset, String name, boolean conservarOriginal) {
        this.inFile = inFile;
        this.pass = pass;
        this.accion = accio;
        this.offset=offset;
    ...
    public void checkAndInit() throws Exception {
        if (!checkAndInit){
            ...
            if (ze == null) {
                lastError = new DataError(DataError.ERROR.ERROR_PASSWORD, bundle.getString("error_password"));
    ...
    private void decrypt()  throws Exception{
            ...
            if (esExtraible){
                ...
                lastError = new DataError(DataError.ERROR.ERROR_EXTRACT, bundle.getString("error_noextract"));
                throw new Exception(lastError.getDescripcion());
            }
    ...
    public void initImage() throws Exception {
        try {
            ...
            lastError = new DataError(DataError.ERROR.ERROR_OTRO, String.format(bundle.getString("error_open"), inFile, e.getMessage()));
            throw new Exception(lastError.getDescripcion());
        }
    }
    ...
    private void openCabezales(boolean usarVistaSegura) throws Exception {
        ...
        if ((version[0] > ((byte) 0x01))) {
            lastError = new DataError(DataError.ERROR.ERROR_VERSION, bundle.getString("error_version"));
            throw new Exception(lastError.getDescripcion());
        }
        ...
        if ((cavezalesActivos[0] & Byte.parseByte("00000100", 2)) == Byte.parseByte("00000000", 2) ){
            if (usarVistaSegura){
                lastError = new DataError(DataError.ERROR.ERROR_OTRO, bundle.getString("error_nosecureview"));
                throw new Exception(lastError.getDescripcion());
            }
        }
        ...
        if ((cavezalesActivos[0] & Byte.parseByte("00000001", 2)) == Byte.parseByte("00000001", 2) ){
            ...
            if (Integer.valueOf(actualDate) > fechaCaducidad) {
                eliminarOriginal();
                lastError = new DataError(DataError.ERROR.ERROR_VENCIMIENTO, bundle.getString("error_defeated"));
                throw new Exception(lastError.getDescripcion());
                //eliminar archivo
            }
        }
        ...
        if (((cavezalesActivos[0] & Byte.parseByte("00010000", 2)) == Byte.parseByte("00010000", 2) ) || (version[0] == ((byte) 0x00))){
            if (LenguajeUtils.isEmpty(pass)){
                lastError = new DataError(DataError.ERROR.ERROR_PASSWORD, bundle.getString("string.empty_password"));
                throw new Exception(lastError.getDescripcion());
            }
            ...
            if ((cavezalesActivos[0] & Byte.parseByte("00000010", 2)) == Byte.parseByte("00000010", 2) ){
                ...
                pass = FileHeader.mergeIdInPassword(pass);
            }
    ...
    public void extraerTodosLosArchivos() throws Exception {
        try {
            ...
            throw new Exception(String.format(bundle.getString("error_unlock"), inFile, e.getMessage()));
        }
